...
//====================================	
	...
	//ELS-1
	CTLSPEC	eg(keyState = KEYINIGNITIONONPOSITION and pitmanArmUpDown_RunnReq = DOWNWARD7 implies (blinkLeft != 0 and blinkRight = 0))
	CTLSPEC	eg(keyState = KEYINIGNITIONONPOSITION and pitmanArmUpDown_RunnReq = UPWARD7 implies (blinkLeft = 0 and blinkRight != 0))
	//ELS-2
	CTLSPEC	eg((keyState = KEYINIGNITIONONPOSITION and (pitmanArmUpDown_RunnReq = DOWNWARD5 or pitmanArmUpDown_RunnReq = DOWNWARD5_LONG)) implies (blinkLeft != 0 and blinkRight = 0))
	CTLSPEC	eg((keyState = KEYINIGNITIONONPOSITION and (pitmanArmUpDown_RunnReq = UPWARD5 or pitmanArmUpDown_RunnReq = UPWARD5_LONG)) implies (blinkLeft = 0 and blinkRight != 0))
	CTLSPEC	eg((keyState = KEYINIGNITIONONPOSITION and (pitmanArmUpDown_RunnReq = DOWNWARD5 and threeBlinkingCycle)) implies ef(blinkLeft = 0 and blinkRight = 0))
	
	CTLSPEC	eg((keyState = KEYINIGNITIONONPOSITION and pitmanArmUpDown_RunnReq = DOWNWARD5) implies e(blinkLeft != 0 , threeBlinkingCycle = true))
	
	CTLSPEC	eg((keyState = KEYINIGNITIONONPOSITION and pitmanArmUpDown_RunnReq = UPWARD5 and threeBlinkingCycle and pitmanArmUpDown=NEUTRAL_UD) implies ex(blinkRight = 0))
	...
	CTLSPEC	eg(hazardWarningSwitchOn_Runn implies (blinkLeft != 0 and blinkRight != 0))
	//
	CTLSPEC	eg((keyState = KEYINIGNITIONONPOSITION and pitmanArmUpDown_RunnReq != NEUTRAL_UD)  implies (blinkLeft != 0 or blinkRight != 0))
	
	
	//ELS-14
	CTLSPEC ag((engineOn and lightRotarySwitch=ON and not overVoltage) implies ax(lowBeamLightingOn))
	
	//ELS-15
	CTLSPEC ag((not ambientLightingAvailable and keyState = KEYINSERTED and lightRotarySwitchPrevious != ON and lightRotarySwitch=ON and not overVoltage) implies ax(lowBeamLeft = 50 and lowBeamRight = 50))
	
	//ELS-16
	CTLSPEC eg((not engineOn_Previous and not engineOn and lightRotarySwitchPrevious != AUTO and lightRotarySwitch = AUTO and not ambientLightingAvailable) implies ax(lowBeamLeft = 0 and lowBeamRight = 0))
	...
	//ELS-17
	CTLSPEC ag((not ambientLightingAvailable and daytimeLight and not engineOn_Previous and engineOn and not lowBeamLightingOn and not overVoltage) implies ax(lowBeamLightingOn))
	CTLSPEC ag((not ambientLightingAvailable and not overVoltage and not parkingLight and daytimeLight and lowBeamLightingOn and not parkingLight and (keyState_Previous = KEYINIGNITIONONPOSITION or keyState_Previous = KEYINSERTED) and keyState = NOKEYINSERTED) implies ax(lowBeamLeft = 0 and lowBeamRight = 0))
	CTLSPEC eg((not parkingLight and daytimeLight and not ambientLightingAvailable and lowBeamLightingOn and not parkingLight and (keyState_Previous = KEYINIGNITIONONPOSITION or keyState_Previous = KEYINSERTED) and keyState = NOKEYINSERTED) implies ax(lowBeamLeft = 0 and lowBeamRight = 0))
	
	//ELS-18
	CTLSPEC ag((lightRotarySwitch = AUTO and engineOn and brightnessSensor < 200 and not overVoltage) implies ax(lowBeamLightingOn))
	CTLSPEC ag((lightRotarySwitch = AUTO and engineOn and lowBeamLightingOn and brightnessSensor > 250 and passed3Sec and not overVoltage) implies ax(not lowBeamLightingOn))
	
	//ELS-19
	CTLSPEC ag((ambientLightingAvailable and ambientLightingConditionOn and not lowBeamLightingOn and not overVoltage) implies ax (lowBeamLightingOn))
	...
	CTLSPEC ag((not(armoredVehicle and darknessModeSwitchOn) and not subVoltage and not overVoltage and lowBeamLightingOn and (currentSpeed > 0 and  currentSpeed <= 100) and ((steeringAngle >= 614 and steeringAngle <= 1022) or (turnRight_RunnReq))) implies ax(corneringLightRight != 0))
	CTLSPEC ag((not(armoredVehicle and darknessModeSwitchOn) and not subVoltage and not overVoltage and lowBeamLightingOn and (currentSpeed > 0 and  currentSpeed <= 100) and ((steeringAngle >= 1 and steeringAngle <= 410) or (turnLeft_RunnReq))) implies ax(corneringLightLeft = 100))
	
	//ELS-27
	CTLSPEC ag((not(armoredVehicle and darknessModeSwitchOn) and not subVoltage and not overVoltage  and lowBeamLightingOn and reverseGear) implies ax(corneringLightLeft = 100 and corneringLightRight = 100))
	
	//ELS-28
	CTLSPEC ag(((ambientLightingAvailable and lowBeamLightingOn and not engineOn) and (passed30Sec) and (parkingLight) and pitmanArmUpDown = UPWARD7 and not subVoltage and not overVoltage ) implies ax(lowBeamRight = 10 and tailLampRight = 10 and lowBeamLeft = 0 and tailLampLeft = 0)) 
	CTLSPEC ag(((ambientLightingAvailable and lowBeamLightingOn and not engineOn) and (passed30Sec) and (parkingLight) and pitmanArmUpDown = DOWNWARD7 and not subVoltage and not overVoltage ) implies ax(lowBeamRight = 0 and tailLampRight = 0 and lowBeamLeft = 10 and tailLampLeft = 10)) 
	
	//ELS-29
	CTLSPEC ag ((brakePedal > 15 and not brakeIsOn and not subVoltage and not overVoltage ) implies ax(brakeLampLeft !=0 and brakeLampRight != 0 and brakeLampCenter != 0))
	CTLSPEC ag ((brakePedal < 5 and brakeIsOn and not subVoltage and not overVoltage ) implies ax(brakeLampLeft =0 and brakeLampRight = 0 and brakeLampCenter = 0))
	...
	//ELS-31
	CTLSPEC ag ((reverseGear and not subVoltage and not overVoltage) implies ax(reverseLight != 0))
